...
#define RASSERT_OVERFLOW() KS_CORE_ASSERT(false, "")
#else
...
// StringBuffer /////////////////////////////////////////////////////////////////
void StringBuffer::init(sizet size, Allocator* allocator) 
{
    if (data) 
    {
    ...
    if (size < 1) 
    {
       KS_CORE_ERROR("ERROR: Buffer cannot be empty!\n");
       return;
    }
    allocator = allocator;
    data = (char*)kalloca(size + 1, allocator);
    KS_CORE_ASSERT(data, "alloca stringBuffer failed!");
    data[0] = 0;
...
void StringBuffer::append_f(const char* format, ...) 
{
    if (current_size >= buffer_size) {
        RASSERT_OVERFLOW();
        KS_CORE_ERROR("Buffer full! Please allocate more size.\n");
        return;
    }
    ...
    {
        RASSERT_OVERFLOW();
        KS_CORE_ERROR("New string too big for current buffer! Please allocate more size.\n");
    }
}
...
// StringArray ////////////////////////////////////////////////////////////
void StringArray::init(u32 size, Allocator* allocator) 
{
    allocator = allocator;
    // Allocate also memory for the hash map
    char* allocated_memory = (char*)allocator->allocate(size + sizeof(FlatHashMap<u64, u32>) + sizeof(FlatHashMapIterator), 1);
...
void StringArray::shutdown() 
{
    ...
    kfree(string_to_index, allocator);
    buffer_size = current_size = 0;
}
...
cstring StringArray::intern(cstring string) 
{
    ...
    const sizet hashed_string = Kenshin::hash_bytes((void*)string, length, seed);
